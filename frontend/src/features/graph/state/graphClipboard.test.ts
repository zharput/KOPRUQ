...
describe('Graph clipboard', () => {
  ...
  it('clones Elastomeric Bearing authoring values and internal connections without candidate cache',()=>{
    const bearingGraph:SpanovaGraph={id:'bearing-clipboard',name:'bearing clipboard',schemaVersion:1,nodes:[
      ...
      {id:'watch',type:'output.watch',name:'Watch',position:{x:500,y:0},parameters:{}},
    ],connections:[{id:'range-bearing',sourceNodeId:'range',sourcePortId:'values',targetNodeId:'bearing',targetPortId:'kx'},{id:'bearing-watch',sourceNodeId:'bearing',sourcePortId:'candidates',targetNodeId:'watch',targetPortId:'value'}]}
    const clone=cloneGraphSelection(copyGraphSelection(bearingGraph,['range','bearing','watch']),40,(()=>{let id=0;return()=>`bearing-copy-${++id}`})())
    expect(clone.nodes.map(node=>node.type)).toEqual(['input.range','substructure.bearing.elastomeric','output.watch'])
    expect(clone.nodes[1].parameters).toMatchObject({kxValue:3000,kxUnit:'kN/m',krxValue:100000,krxUnit:'kNm/rad'})
    expect(clone.nodes[1].parameters).not.toHaveProperty('candidates')
    expect(clone.connections.map(edge=>[edge.sourceNodeId,edge.targetNodeId])).toEqual([['bearing-copy-1','bearing-copy-2'],['bearing-copy-2','bearing-copy-3']])
  })
