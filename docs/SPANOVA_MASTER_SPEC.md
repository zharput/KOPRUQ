> This is the authoritative master specification for SPANOVA. Maintain
...
for a new professional bridge engineering software platform called SPANOVA.
...
SPANOVA is intended to become a professional Computational and Generative Bridge
Design platform.

It is NOT intended to replace ALLPLAN Civil, MIDAS Civil, SCIA Engineer or other
structural analysis/BIM software.

Instead, SPANOVA will become the engineering intelligence, automation, generation
...
SPANOVA must answer the question:
...
Therefore SPANOVA shall focus on:
...
SPANOVA shall NOT initially contain its own finite-element solver.
...
SPANOVA shall be a standalone professional desktop engineering application.
...
The user should primarily work inside SPANOVA.
...
SPANOVA shall eventually support two operating modes.
...
SPANOVA generates alternatives.
...
The SPANOVA domain model shall eventually include concepts such as:
...
A typical SPANOVA design-space definition may contain:
...
SPANOVA must use engineering knowledge to eliminate unreasonable alternatives before
...
property components of SPANOVA.
...
SPANOVA generates a selected BridgeAlternative.

Spanova.Allplan converts it into an ALLPLAN-compatible Tcl/API representation.
...
ALL ALLPLAN-specific code must remain inside the Spanova.Allplan project.
...
SPANOVA Core must not know which solver is being used.
...
SPANOVA shall eventually evaluate:
...
Eventually SPANOVA shall support multi-objective optimization.
...
The AI layer converts this request into deterministic SPANOVA parameters.
...
SPANOVA shall have its own project format.

Example:

VIA35.spanova

SPANOVA project data must NOT simply be an ALLPLAN file.
...
Implement the first useful SPANOVA capability.
...
Implement initial Spanova.Allplan adapter.
...
A bridge selected in SPANOVA can be reproduced correctly in ALLPLAN Civil.
...
SPANOVA can associate structural-analysis results with a BridgeAlternative.
...
SPANOVA identifies multiple Pareto-optimal bridge alternatives.
...
Only after the workflow is proven, create the polished SPANOVA interface.
...
Keep ALLPLAN-specific logic out of Spanova.Core.
...
DO NOT write the entire SPANOVA application now.
...
3. Propose the SPANOVA software architecture.
...
We will build SPANOVA incrementally.
