...
export default function NodeInspector({ node, selectedNodes = [], states, errors, outputs, resolvedInputs = {}, previewInputs = {}, connections = [], concreteMaterial, projectUnits, onNodeChange, onParameterChange }: Props) {
  ...
  return <div className="spn-graph-inspector spn-general-node-inspector">
    <InspectorSection title="GENERAL"><label>Node name<input className="spn-input" value={node.name} onChange={event => onNodeChange(node.id, { name: event.target.value })} /></label>{node.type !== 'input.notes' && <div className="spn-graph-inspector-row"><span>Type</span><strong>{definition.label}</strong></div>}<div className="spn-graph-inspector-row"><span>Execution</span><strong className={`state-${states[node.id] ?? 'idle'}`}>{(states[node.id] ?? 'idle').toUpperCase()}</strong></div>{errors[node.id] && <p className="spn-graph-inspector-error">{errors[node.id]}</p>}</InspectorSection>
    {parameters.length > 0 && <InspectorSection title="PARAMETERS / VALUE">{parameters.filter(parameter => node.type !== 'input.length' || parameter.key === 'mode' || (String(node.parameters.mode ?? 'single') === 'single' ? parameter.key === 'value' : ['min','max','step'].includes(parameter.key))).map(parameter => {
      ...
      return <InspectorParameterRow key={parameter.key} label={humanParameterLabel(parameter.label)} connected={isConnected && <small>Connected · {input === undefined ? 'waiting for value' : formatGraphValue(input)}</small>}>{parameter.dataType === 'boolean' ? <input type="checkbox" checked={Boolean(node.parameters[parameter.key])} disabled={isConnected} onChange={event => onParameterChange(node.id, parameter.key, event.target.checked)} /> : parameter.dataType === 'select' ? <select className="spn-input" aria-label={humanParameterLabel(parameter.label)} value={String(node.parameters[parameter.key] ?? units[0]?.value ?? '')} disabled={isConnected || !units.length} onChange={event => {onParameterChange(node.id, parameter.key, event.target.value);if(parameter.key==='quantityKind'){const kind=event.target.value as QuantityKind,preferred=projectUnits?.[kind],unit=unitsForKind(kind).find(item=>item.id===preferred||item.label===preferred)??unitsForKind(kind)[0];onParameterChange(node.id,'unit',unit.id)}}}><option value="" disabled>Select</option>{units.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select> : parameter.dataType === 'string' ? <input className="spn-input" aria-label={humanParameterLabel(parameter.label)} value={String(node.parameters[parameter.key] ?? '')} onChange={event => onParameterChange(node.id, parameter.key, event.target.value)} /> : <EditableNumericInput value={displayValue} integer={parameter.dataType === 'integer'} ariaLabel={humanParameterLabel(parameter.label)} disabled={isConnected} onCommit={value => onParameterChange(node.id, parameter.key, node.type === 'input.length' ? convertQuantity(Number(value), projectUnits?.length ?? 'm', 'm') : quantityParameterInternalValue(node, parameter.key, value, projectUnits))} />}</InspectorParameterRow>
    })}</InspectorSection>}
    {incoming.length > 0 && <InspectorSection title="CONNECTED VALUES">{incoming.map(item => <div className="spn-graph-inspector-row" key={item.label}><span>{item.label}</span><strong>{item.value === undefined ? 'Waiting for value' : formatGraphValue(item.value)}</strong></div>)}</InspectorSection>}
    {node.type.startsWith('material.') && <InspectorSection title="MATERIAL"><div className="spn-graph-inspector-row"><span>Grade</span><strong>{material?.name ?? String(node.parameters.materialId ?? 'Not selected')}</strong></div><MaterialProperties material={material} /></InspectorSection>}
    {(node.type.startsWith('output.') || previewValue !== undefined) && <InspectorSection title="RESULT / PREVIEW">{previewValue === undefined ? <small>{incoming.length ? 'Waiting for a connected value.' : 'Connect a value to preview this node.'}</small> : <GraphValuePreview value={previewValue} />}</InspectorSection>}
  </div>
}
