...
import type { GraphValue, PierCandidate, KopruqGraph, KopruqNode } from '../domain/types'
import { toReactFlowNodes } from '../adapters/reactFlowAdapter'
import BaseNode from './BaseNode'

function renderWatch(value: GraphValue) {
  const graph: KopruqGraph = { id: 'g', name: 'watch test', schemaVersion: 1, nodes: [{ id: 'watch', type: 'output.watch', name: 'Watch', position: { x: 0, y: 0 }, parameters: {} }], connections: [] }
  const [node] = toReactFlowNodes(graph, { outputs: { watch: { value } }, onParameterChange: vi.fn() })
  return render(<ReactFlowProvider><BaseNode data={node.data} selected={false} id={node.id} type="kopruq" dragging={false} zIndex={0} positionAbsoluteX={0} positionAbsoluteY={0} isConnectable draggable deletable selectable sourcePosition={Position.Right} targetPosition={Position.Left} /></ReactFlowProvider>)
}

function renderCanvasNode(type: string) {
  const graph: KopruqGraph = { id: `g-${type}`, name: type, schemaVersion: 1, nodes: [{ id: 'node', type, name: type, position: { x: 0, y: 0 }, parameters: { value: type === 'input.integer' ? 5 : 10 } }], connections: [] }
...
describe('Watch node rendering', () => {
  it('shows generic Integer List values and does not expose the technical port type on canvas',()=>{
    const node:KopruqNode={id:'ints',type:'input.integer-list',name:'Integer List',position:{x:0,y:0},parameters:{valuesText:'3, 4, 5'}}
    const graph:KopruqGraph={id:'g',name:'integer list',schemaVersion:1,nodes:[node],connections:[]}
    const [projected]=toReactFlowNodes(graph,{onParameterChange:vi.fn()})
    const {container}=render(<ReactFlowProvider><BaseNode data={projected.data} selected={false} id={projected.id} type="kopruq" dragging={false} zIndex={0} positionAbsoluteX={0} positionAbsoluteY={0} isConnectable draggable deletable selectable sourcePosition={Position.Right} targetPosition={Position.Left}/></ReactFlowProvider>)
  ...
  it('shows immediately available material and number source previews without Run', () => {
    ...
    render(<ReactFlowProvider><BaseNode data={watch.data} selected={false} id={watch.id} type="kopruq" dragging={false} zIndex={0} positionAbsoluteX={0} positionAbsoluteY={0} isConnectable draggable deletable selectable sourcePosition={Position.Right} targetPosition={Position.Left} /></ReactFlowProvider>)
    expect(screen.getByText('Concrete C35/45')).toBeInTheDocument()

    const numberGraph: KopruqGraph = { id: 'g2', name: 'number preview', schemaVersion: 1, nodes: [{ id: 'number', type: 'input.number', name: 'Number', position: { x: 0, y: 0 }, parameters: { value: 8 } }, { id: 'watch2', type: 'output.watch', name: 'Watch', position: { x: 200, y: 0 }, parameters: {} }], connections: [{ id: 'e2', sourceNodeId: 'number', sourcePortId: 'value', targetNodeId: 'watch2', targetPortId: 'value' }] }
  ...
  it('edits the concrete grade directly on the canvas', async () => {
    ...
    const graph: KopruqGraph = { id: 'g', name: 'material selector', schemaVersion: 1, nodes: [{ id: 'concrete', type: 'material.concrete', name: 'Concrete', position: { x: 0, y: 0 }, parameters: { materialId: 'C40/50' } }], connections: [] }
    const [node] = toReactFlowNodes(graph, { onParameterChange: onChange })
    const { container } = render(<ReactFlowProvider><BaseNode data={node.data} selected={false} id={node.id} type="kopruq" dragging={false} zIndex={0} positionAbsoluteX={0} positionAbsoluteY={0} isConnectable draggable deletable selectable sourcePosition={Position.Right} targetPosition={Position.Left} /></ReactFlowProvider>)
  ...
  it('renders editable Range Start/End/Step, an immediate value count, and disables a connected editor', async () => {
    const onChange=vi.fn(),graph:KopruqGraph={id:'range',name:'Range',schemaVersion:1,nodes:[
    ...
    render(<ReactFlowProvider><BaseNode data={range.data} selected={false} id={range.id} type="kopruq" dragging={false} zIndex={0} positionAbsoluteX={0} positionAbsoluteY={0} isConnectable draggable deletable selectable sourcePosition={Position.Right} targetPosition={Position.Left} /></ReactFlowProvider>)
...
describe('canvas port labels and structural family theme', () => {
  ...
  it('assigns all current Pier family nodes to the shared structural family theme', () => {
    const graph: KopruqGraph = { id: 'pier', name: 'Pier', schemaVersion: 1, nodes: [{ id: 'pier', type: 'substructure.pier.circular', name: 'Pier', position: { x: 0, y: 0 }, parameters: {} }], connections: [] }
    const [node] = toReactFlowNodes(graph, { onParameterChange: vi.fn() })
    render(<ReactFlowProvider><BaseNode data={node.data} selected={false} id={node.id} type="kopruq" dragging={false} zIndex={0} positionAbsoluteX={0} positionAbsoluteY={0} isConnectable draggable deletable selectable sourcePosition={Position.Right} targetPosition={Position.Left} /></ReactFlowProvider>)
