...
function girderUnitValue(value:number,unit:UnitId){return Number(quantityFromCanonical(value,'length',unit).toPrecision(15))}
function canonicalGirderLength(value:number,unit:UnitId){return localLength(value,unit).value}
function girderNode(kind:'PRECAST'|'STEEL'):NodeDefinition {
  ...
  return {type:`structural.girder.${kind.toLowerCase()}`,label:steel?'Steel Girder':'Precast Girder',category:'STRUCTURAL_FAMILY',description:`Generate deterministic ${steel?'welded steel I':'precast I'} girder section candidates.`,inputs,outputs:[{id:'candidates',label:'Candidates',type:'girderCandidate[]'}],parameterSchema,engineeringInspector:{schematic:'girder',parameterOrder:[...fields,'material','preferredSpan','applicableSpanRange']},createDefaultParameters:prefs=>{const unit=defaultUnit('length',prefs),p:Record<string,GraphParameterValue>={};for(const key of fields){p[key + 'Value']=girderUnitValue(girderDefault(kind,key),unit);p[key + 'Unit']=unit}Object.assign(p,{familyId:steel?'SG-01':'PG-200',preferredSpanValue:girderUnitValue(steel?60:40,unit),preferredSpanUnit:unit,minSpanValue:girderUnitValue(30,unit),minSpanUnit:unit,maxSpanValue:girderUnitValue(90,unit),maxSpanUnit:unit,materialId:steel?'S355':'C40/50'});return p},validateParameters:p=>[...fields.flatMap(key=>numberParameter(p,`${key}Value`)),...(steel&&typeof p.materialId==='string'&&STRUCTURAL_STEEL_OPTIONS.some(item=>item.value===p.materialId)?[]:steel?['Select a supported Structural Steel grade.']:[])],executor:({node,inputs})=>{const geometry=Object.fromEntries(fields.map(key=>[key,inputs[key]??localLength(node.parameters[`${key}Value`] as number,node.parameters[`${key}Unit`] as UnitId)]));const material=(inputs.material??{domainType:steel?'StructuralSteelMaterial':'ConcreteMaterial',id:String(node.parameters.materialId??''),name:String(node.parameters.materialId??''),properties:{}}) as MaterialValue;const args={geometry,material,preferredSpan:canonicalGirderLength(Number(node.parameters.preferredSpanValue),String(node.parameters.preferredSpanUnit) as UnitId),minSpan:canonicalGirderLength(Number(node.parameters.minSpanValue),String(node.parameters.minSpanUnit) as UnitId),maxSpan:canonicalGirderLength(Number(node.parameters.maxSpanValue),String(node.parameters.maxSpanUnit) as UnitId),familyId:String(node.parameters.familyId)};return {candidates:(steel ? generateSteelGirderCandidates({H:geometry.H as never,Btf:geometry.Btf as never,ttf:geometry.ttf as never,Bbf:geometry.Bbf as never,tbf:geometry.tbf as never,tw:geometry.tw as never,material,preferredSpan:args.preferredSpan,minSpan:args.minSpan,maxSpan:args.maxSpan,familyId:args.familyId}) : generatePrecastGirderCandidates(args as never)).candidates as unknown as GraphValue}}}
}
...
const definitions:NodeDefinition[]=[
 ...
 {type:'input.notes',label:'Notes',category:'OUTPUT',description:'Add an informational note or explanation to the graph.',inputs:[],outputs:[],parameterSchema:[{key:'text',label:'Notes',dataType:'string'}],createDefaultParameters:()=>({text:''}),validateParameters:()=>[],executor:()=>({})},
