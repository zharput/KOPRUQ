...
export interface EngineeringInspectorSchema { schematic: 'pier' | 'pier-cap' | 'foundation' | 'bearing' | 'girder' | 'superstructure'; parameterOrder: readonly string[] }
...
const definitions:NodeDefinition[]=[
 ...
 ...girderDefinitions, superstructureDefinition,
...
export function previewDesignOutput(node:SpanovaNode,portId:string,inputs:Record<string,GraphValue>):GraphValue|undefined {
  ...
  if(node.type==='structural.superstructure'&&portId==='candidates'){
    ...
    const girder=inputs.girder
    if(!Array.isArray(girder)) return [] as unknown as GraphValue
    return generateSuperstructureCandidates({girders:girder as unknown as import('../domain/girderCandidates').GirderCandidate[],deckWidth:(inputs.deckWidth??localLength(Number(node.parameters.deckWidthValue),String(node.parameters.deckWidthUnit))) as never,girderCount:(inputs.girderCount??node.parameters.girderCount) as never,girderSpacing:(inputs.girderSpacing??localLength(Number(node.parameters.girderSpacingValue),String(node.parameters.girderSpacingUnit))) as never,deckSlabThickness:(inputs.deckSlabThickness??localLength(Number(node.parameters.deckSlabThicknessValue),String(node.parameters.deckSlabThicknessUnit))) as never,deckConcrete}).candidates as unknown as GraphValue
  }
  return undefined
}
