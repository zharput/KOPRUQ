...
export default function NodeLibrary({ onAdd }: { onAdd: (type: string) => void }) {
  ...
  return <div className="spn-graph-library">
    ...
    {CATEGORIES.map((category) => {const items=definitions.filter(definition=>definition.category===category);return <section className={`spn-graph-library-group ${getNodeTheme(category).className}`} key={category}><h3>{getNodeCategoryLabel(category)}</h3>{category==='STRUCTURAL_FAMILY'?<><LibrarySubgroup title="Pier" items={items.filter(item=>item.type.startsWith('substructure.pier.')&&!item.type.startsWith('substructure.pier-cap.'))} onAdd={onAdd}/><LibrarySubgroup title="Pier Cap" items={items.filter(item=>item.type.startsWith('substructure.pier-cap.'))} onAdd={onAdd}/><LibrarySubgroup title="Foundation" items={items.filter(item=>item.type.startsWith('substructure.foundation.'))} onAdd={onAdd}/><LibrarySubgroup title="Bearing" items={items.filter(item=>item.type.startsWith('substructure.bearing.'))} onAdd={onAdd}/><LibrarySubgroup title="Girder" items={items.filter(item=>item.type.startsWith('structural.girder.'))} onAdd={onAdd}/><LibrarySubgroup title="Superstructure" items={items.filter(item=>item.type==='structural.superstructure')} onAdd={onAdd}/></>:items.map((definition) => <LibraryItem key={definition.type} definition={definition} onAdd={onAdd}/>)}</section>})}
