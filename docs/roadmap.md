...
Milestones and gating rules are defined in `docs/KOPRUQ_MASTER_SPEC.md`
...
| P10 | Professional "Design Cockpit" React UI (Project/Geometry/Design Space/Generate/Analyze/Optimize/Results/Report) - see `Kopruq_menü.jpg` / `Kopruq_menü_dark.jpg` in the repo root for the target look | **Shell done** (this session, out of milestone order - see notes below) - dashboard widgets (map, stats, Pareto) not started |
| P11 | AI Service (Python/FastAPI): natural-language -> deterministic KOPRUQ parameters; never a structural-safety authority | Not started |
...
existing plan came out of this: (1) KOPRUQ gets its own native
...
| Native analysis engine (addendum P) | `kopruq-analysis-engine`: linear-elastic direct stiffness method, 3D frame elements, elastic-link bearings, `POST /api/fast-solver/solve`, KOPRUQ Fast Solver frontend screen | **Done** (this session) - validated against closed-form results and the engineer's own 5-span frame-bridge test case; see `docs/architecture.md`'s matching section |
...
- [x] `backend/bridge-core/src/main/java/com/kopruq/bridgecore/`:
...
- [x] `frontend/src/App.tsx` rewritten: plain form with the same 9
      ...
      removed.
- [x] `.claude/launch.json` added (`kopruq-frontend`: `npm --prefix
...
- [x] `backend/rules-engine` (`com.kopruq.rules`): `EngineeringRule`
...
- [x] `frontend/src-tauri/` scaffolded (`npx tauri init`), verified
      working (`npm run tauri dev` opened a real native "KOPRUQ" window,
...
- [x] **Permanent Java implementation written**, after a brief pause to
      ...
      frontend involved either way):
      - `backend/analysis-api` (`com.kopruq.analysis`): `AnalysisEngine`
      ...
      - `backend/midas-adapter` (`com.kopruq.midas`): `MidasApiClient`
...
mockup shared in chat (same as `Kopruq_menü.jpg`/`Kopruq_menü_dark.jpg`
already referenced above).

- [x] `frontend/src/App.tsx` rebuilt as the shell: `Sidebar` (12 nav
      items, real KOPRUQ logo), `TopBar` (project picker/search/
...
- [x] **No bridge photograph** - the mockup's hero image is a real photo
      ...
      (arcs + deck + pier lines, echoing the KOPRUQ logo's own
...
- [x] `PierFamiliesPanel.tsx` (new) - 'pier-families' is real: 4
      ...
      "use in KOPRUQ analyses" checkbox, a redrawn SVG diagram, and a
...
## Native analysis engine - kopruq-analysis-engine (2026-09-10, same session)
...
- [x] `analysis-engine` module: frame element stiffness, axis
      ...
      `KopruqAnalysisEngine implements AnalysisEngine`.
...
- [x] `com.kopruq.api.fastsolver` package + `POST /api/fast-solver/solve`
      + `FastSolverPanel.tsx` ('kopruq-fast-solver' now real).
...
- **Traffic load (LM1)**: MIDAS's own Eurocode vehicle library
  ...
  characteristic tandem/UDL values - KOPRUQ does not compute or invent
...
engine is now the native `kopruq-analysis-engine` (linear-elastic, 1D
...
(e.g. `Kopruq.Generative.AlternativeGenerator`'s uniform-span
...
- [x] `/` and `/home` redirect to `/project`; the KOPRUQ logo opens
...
- [x] Serve the original `Kopruq_logo.png` unchanged with contain sizing and preserve logo navigation.
