...
function AbutmentCompactNodeShell({ data, selected, definition }: { data: FlowGraphNode['data']; selected: boolean; definition: NodeDefinition }) {
  ...
  return <div className={`spn-graph-node spn-engineering-node spn-abutment-node category-structural-family category-abutment${selected ? ' is-selected' : ''}`}><NodeHeader definition={definition} type={data.node.type} /><div className="spn-engineering-inputs">{definition.inputs.slice(0, 3).map(port => <EngineeringInputRow key={port.id} port={port} definition={definition} data={data} />)}<div className="spn-engineering-group-heading">ABUTMENT GEOMETRY</div>{fields.map(field)}<div className="spn-engineering-group-heading">SEISMIC BLOCK</div>{field('sei_u')}<div className="spn-engineering-derived"><span>sei_w</span><b>{candidate?.seismic?.seiW === undefined ? '—' : `${candidate.seismic.seiW}`}</b></div><div className="spn-engineering-derived"><span>STATUS</span><b>{status}</b></div>{candidate?.seismic?.message && <div className="spn-graph-inspector-warning">{candidate.seismic.message}</div>}</div><div className="spn-engineering-outputs">{definition.outputs.map(port => <div className="spn-engineering-output-row" key={port.id}><span className="spn-engineering-output-label">{port.label}</span><span className="spn-engineering-output-value">{count} candidates</span><Handle type="source" position={Position.Right} id={port.id} isConnectable /></div>)}</div><div className="spn-graph-node-state">{data.executionState.toUpperCase()}</div></div>
}
