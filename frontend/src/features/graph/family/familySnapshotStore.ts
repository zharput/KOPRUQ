...
export function publishFamilySnapshots(items: readonly FamilyCalculationSnapshot[]) { for (const item of items) snapshots.set(key(item.bridgeId, item.graphDocumentId, item.sourceNodeId, item.familyCategory), item); if (typeof window !== 'undefined') window.dispatchEvent(new Event('kopruq:family-snapshots-changed')) }
